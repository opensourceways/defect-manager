...
var (
	...
	regexpOfItems = map[string]*regexp.Regexp{
		...
		itemComponents:                    regexp.MustCompile(`(缺陷所属软件及版本号)[】][(（]必填，如kernel-5.10.0-60.138.0.165[)）]\*\*([\s\S]*?)\*\*【环境信息`),
		...
		itemTitleComponents:               regexp.MustCompile(`(缺陷所属软件及版本号)[】][(（]必填，如kernel-5.10.0-60.138.0.165[)）]([\s\S]*?)\*\*【环境信息`),
